...
describe('Sign up', () => {
    ...
    it('Should succeed with correct data', async () => {
        ...
        const createUserSpy = vi
            ...
            .mockImplementation(async (newUser) => result.ok({
            id: userId,
            ...newUser
        }));
        const token = 'foo.bar';
        ...
        const createSessionSpy = vi
            ...
            .mockImplementation(async () => result.ok(option.some({
            session,
            token
        })));
...
describe('Sign in', () => {
    ...
    it('Should succeed with correct data', async () => {
        ...
        const signInSpy = vi
            ...
            .mockImplementation(async (username) => result.ok({
            id: userId,
            username
        }));
        const token = 'foo.bar';
        ...
        expect(sessionStoreSpy).toHaveBeenCalledExactlyOnceWith('foo.bar', JSON.stringify({
    ...
    it('Should fail with user not found, 404', async () => {
        vi.spyOn(UserHandler.prototype, 'findUserByUsername').mockImplementation(async () => result.ok(undefined));
        const res = await app.request(path, {
            method: 'post',
            headers: {
                'Content-Type': 'application/json'
